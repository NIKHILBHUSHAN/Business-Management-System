...
public class ShopManagementSystem {
    ...
    public static void showDashboard() {
        ...
        // Actions
        addBtn.addActionListener(e -> addOrUpdateProduct());
        sellBtn.addActionListener(e -> { sellProduct(); updateTotalRevenue(); saveData(); });
        deleteBtn.addActionListener(e -> { deleteProduct(); saveData(); });
        undoBtn.addActionListener(e -> { undoLastSale(); updateTotalRevenue(); saveData(); });
        reportBtn.addActionListener(e -> showDailyReport());
        inventoryBtn.addActionListener(e -> viewInventory());
        searchBtn.addActionListener(e -> searchProduct());
    }

    // ===== PRELOAD PRODUCTS (if file empty) =====
    public static void preloadProducts() {
        for (int i = 1; i <= 30; i++) {
            products.add(new Product(i, "Product" + i, 100 + i * 10, 5, 10));
        }
    ...
    public static void showDailyReport() {
        ...
        saveButton.addActionListener(e -> {
            try {
                FileWriter fw = new FileWriter("DailyReport.txt");
                fw.write(sb.toString());
                fw.close();
                JOptionPane.showMessageDialog(null, "Daily report saved as DailyReport.txt");
            } catch (Exception ex) {
        ...
        JOptionPane.showMessageDialog(null, panel, "Daily Report", JOptionPane.INFORMATION_MESSAGE);
    }
    ...
    public static void saveData() {
        try {
            ObjectOutputStream oos1 = new ObjectOutputStream(new FileOutputStream(PRODUCT_FILE));
            oos1.writeObject(products);
            oos1.close();
            ObjectOutputStream oos2 = new ObjectOutputStream(new FileOutputStream(SALES_FILE));
            oos2.writeObject(dailySales);
            oos2.close();
        } catch (Exception e) {
    ...
    public static void loadData() {
        try {
            ObjectInputStream ois1 = new ObjectInputStream(new FileInputStream(PRODUCT_FILE));
            products = (ArrayList<Product>) ois1.readObject();
            ois1.close();
        } catch (Exception e) {
            preloadProducts(); // load default if file not found
        }
        try {
            ObjectInputStream ois2 = new ObjectInputStream(new FileInputStream(SALES_FILE));
            dailySales = (ArrayList<Sale>) ois2.readObject();
            ois2.close();
        } catch (Exception e) {
